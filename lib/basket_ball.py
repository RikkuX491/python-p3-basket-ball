...
def num_points_per_game(player_name):
    return get_all_players().get(player_name)["points_per_game"]

def player_age(player_name):
    return get_all_players().get(player_name)["age"]
...
def player_stats(player_name):
    return get_all_players().get(player_name)

def average_rebounds_by_shoe_brand():
    all_shoe_brands = {}
    for team in ['home', 'away']:
        for player in game_dict()[team]['players']:
            if(all_shoe_brands.get(player['shoe_brand'])):
                all_shoe_brands.update({player['shoe_brand']: [*all_shoe_brands[player['shoe_brand']], player['rebounds_per_game']]})
            else:
                all_shoe_brands.update({player['shoe_brand']: [player['rebounds_per_game']]})

    for shoe_brand in all_shoe_brands:
        print(f"{shoe_brand}:  {(sum(all_shoe_brands[shoe_brand]) / len(all_shoe_brands[shoe_brand])):.2f}")